...
- Run `/code-review` before opening the pull request, not after, and fix what it finds. Add a Copilot review only for a large or multi-file change; a small targeted one does not need it. Update the description when a later fix changes something it claims.
