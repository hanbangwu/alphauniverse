...
- Never write linter ignore rules as comments, configs, or any other expressions. The exception is vendored code we do not author: `frontend/src/lib/components/ui` is regenerated by `bun run shadcn`, so a fix there does not survive.
...
Specific for this project: don't edit `frontend/src/lib/components/ui`.
