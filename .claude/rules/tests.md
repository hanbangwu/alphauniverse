...
- A module that imports torch is tested with AION and its codecs built with random weights from configs vendored in `tests/`; no test downloads weights. Its test file starts with `pytest.importorskip("torch")`, so the default suite runs without the `build` group, and a CI job that installs the group runs it.
