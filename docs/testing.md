...
The run writes its report to `docs/benchmarks/latest.json`, replacing the previous one; commit it before the next run. The report records the commits, dataset revision of each round, date, the Modal spec of server and client, the thread configuration, and the CPU identity (vendor, family, model and model name) of the client and stage containers. It names the best version: the lowest warm `search()` time at 32 matches (`total_p50_ms`), averaged over the version's rounds, among versions whose rounds all succeeded. A stored best commit that is not in the clone is skipped and noted. It never runs in CI.
