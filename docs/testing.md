...
- `test_approximate_ranking_agrees_with_exact` compares the ordered top 2 of `search()` with a brute-force ranking over every token. The brute force follows `search()` except for the candidate step, scores the float32 embeddings rather than the index's fp16 copies, and holds the whole corpus in memory, so it only runs at fixture scale. Asking for 2 of the 11 other galaxies makes the candidate step choose, so a wrong candidate set or a wrong query direction changes the result, which comparing the whole corpus as a set would miss. `test_fixture_separates_the_ranked_scores` asserts that the exact scores in that top 2, and the gap to the third, differ by more than `SEPARATION` (1e-3), well above the index's fp16 rounding, so a fixture whose clusters drift together fails there rather than making the comparison flaky.
