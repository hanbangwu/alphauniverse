...
That gives a working order: the `index-mmap` measurement and `production-recall` first, which between them make the `patch-array` + `index-compression` design possible.
